...
init_checkpoint_log()
{
	...
	if ((c_log = (struct checkpoint_log *)pmem_map_file(
		     "/mnt/pmem/pmem_checkpoint.pm", PMEM_LEN, PMEM_FILE_CREATE,
...
/*void init_checkpoint_log(){
  non_checkpoint_flag = 1;
  settings.pm_pool = pmemobj_create("/mnt/mem/checkpoint.pm", "checkpoint",
...
shift_to_left(int variable_index)
{
	...
	for (int i = 0; i < MAX_VERSIONS - 1; i++) {
		...
		// pmem_memcpy_persist(checkpoint_file_address, c_log,
		// sizeof(struct checkpoint_log));
		// pmem_memcpy_persist(checkpoint_file_curr,
		// c_log->c_data[variable_index].data[0], size);
		// c_log->c_data[variable_index].data[0] = checkpoint_file_curr;
		if (is_pmem)
			pmem_persist(checkpoint_file_address, mapped_len);
		else
			pmem_msync(checkpoint_file_address, mapped_len);
		// checkpoint_file_curr = (void
		// *)((uint64_t)checkpoint_file_curr + size);
	     ...
	     const void *data_address, uint64_t offset)
{
	...
	if (variable_index == 0 && variable_count == 0) {
		...
		// printf("file address is %p\n", checkpoint_file_address);
		pmem_memcpy_persist(checkpoint_file_curr,
				    c_log->c_data[variable_index].data[0],
				    size);
		c_log->c_data[variable_index].data[0] = checkpoint_file_curr;
		// pmem_memcpy_persist(checkpoint_file_address, c_log,
		// sizeof(struct checkpoint_log));
		...
		// printf("new c log variable count is %d\n",
		...
		// sizeof(struct checkpoint_log));
		// pmem_memcpy_persist(checkpoint_file_curr,
		// c_log->c_data[variable_index].data[data_index], size);
		pmem_memcpy_persist(
			checkpoint_file_curr,
			c_log->c_data[variable_index].data[data_index], size);
		c_log->c_data[variable_index].data[data_index] =
		...
		checkpoint_file_curr =
			(void *)((uint64_t)checkpoint_file_curr + size);
		// struct checkpoint_log *new_c_log = (struct checkpoint_log
		// *)checkpoint_file_address;
	}
	//}TX_END
	non_checkpoint_flag = 0;
